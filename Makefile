...
miniscript.js: $(HEADERS) $(SOURCES) js_bindings.cpp
	em++ -O3 -g0 -Wall -std=c++17 -fno-rtti -flto -Ibitcoin -I. $(SOURCES) js_bindings.cpp -s WASM=1 -s FILESYSTEM=0 -s ENVIRONMENT=web -s DISABLE_EXCEPTION_CATCHING=0 -s EXPORTED_FUNCTIONS='["_miniscript_compile","_miniscript_analyze","_miniscript_to_json","_malloc","_free"]' -s EXPORTED_RUNTIME_METHODS='["cwrap","UTF8ToString"]' -o miniscript.js
